...
<?php
    ...
    $nomeUsuario = $sobrenomeUsuario = $cidadeUsuario = $enderecoUsuario = $generoUsuario = $numeroEnderecoUsuario = $cepUsuario = $complementoUsuario = $emailUsuario = $senhaUsuario = $confirmarSenhaUsuario = "";
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($_POST["enderecoUsuario"])){
            echo "<div class='alert alert-warning text-center'>O campo <strong>ENDEREÇO / RUA</strong> é obrigatório!</div>";
            $erroPreenchimento = true;
        }
        else {
        ...
        //Validação do campo generoUsuario utilizando a função empty
        if(empty($_POST["generoUsuario"])){
            echo "<div class='alert alert-warning text-center'>O campo <strong>GENERO</strong> é obrigatório!</div>";
            $erroPreenchimento = true;
        }
        else {
            $generoUsuario = filtrar_entrada($_POST["generoUsuario"]);
        }
        ...
        if(empty($_POST["numeroEnderecoUsuario"])){
            ...
            $numeroEnderecoUsuario = filtrar_entrada($_POST["numeroEnderecoUsuario"]);
        }
        ...
        if(empty($_POST["cepUsuario"])){
            ...
            $cepUsuario = filtrar_entrada($_POST["cepUsuario"]);
        }
        ...
        if(empty($_POST["complementoUsuario"])){
            ...
            $complementoUsuario = filtrar_entrada($_POST["complementoUsuario"]);
        }
        ...
        if(!$erroPreenchimento){
            ...
            $inserirUsuario = "INSERT INTO Usuarios (nomeUsuario, sobrenomeUsuario, generoUsuario, enderecoUsuario, cidadeUsuario, numeroEnderecoUsuario, cepUsuario, complementoUsuario, emailUsuario, senhaUsuario)
            VALUES ('$nomeUsuario', '$sobrenomeUsuario', '$generoUsuario', '$enderecoUsuario', '$cidadeUsuario', '$numeroEnderecoUsuario', '$cepUsuario', '$complementoUsuario', '$emailUsuario', '$senhaUsuario')";
